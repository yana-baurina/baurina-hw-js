...
//function
function menu(humburgers, fries){
    if(humburgers >= 4 && fries >= 1){
        return('Ми поїли')
    }else{
...
function checkPrice1(price1){
    if(!(price1 >= 1000 && price1 <= 1900)){
        return('значення ціни товару не знаходиться в діапазоні')
    };
}
console.log(checkPrice1(1901));
...
//
function checkSeasonOfTheYear(season){
    if(season == 1){
        return('winter')
    }else if(season == 2){
        return('spring')
...
console.log(checkSeasonOfTheYear(5));
...
console.log(getMiddleNumber(3, 5, 1));
...
function calculate(d, operation, e){
    switch(operation){
        case '+':
            result = d + e;
            break;
...
console.log(removeVowels("MolokO"))
